...
ofVec3f Boid::newseparation(std::vector<Boid *> &otherBoids)
{
	...
	for (int i = 0; i < otherBoids.size(); i++)
	{	
		if(position.distance(otherBoids[i]->getPosition()) < separationThreshold*2)
		{
			ofVec3f v = position -  otherBoids[i]->getPosition();
			v.normalize();
			return v;
...
{
	velocity += separationWeight*separation(otherBoids); //v = v + 
...
void Boid::newupdate(std::vector<Boid*>& otherHunters, std::vector<Boid*>& otherBoids, ofVec3f& min, ofVec3f& max)
{
	velocity += separationWeight * newseparation(otherBoids); //v = v + 
	velocity += cohesionWeight * cohesion(otherBoids);
	velocity += alignmentWeight * alignment(otherBoids);
...
void Boid::draw()
{
	ofSetColor(10, 12, 255);
	ofCircle(position.x, position.y, 2);
}

void Boid::newdraw()
{
    ofSetColor(255, 40, 12);
    ofCircle(position.x + 150, position.y + 150, 4);
}
